...
export function buildMasterclassEmail(input: { name?: string | null; email: string }) {
  ...
  const appLabel = links.appUrl ? 'Descargar la app' : 'Solicitar demo';
  const appHref = links.appUrl || links.signupUrl;

  const subject = 'Tu recurso gratuito ya esta listo';
  const preview =
    'Acceso inmediato a tu guia para convertir mas leads en ventas con un sistema comercial claro.';
  ...
  const html = `
    <div style="background:#f8fafc;padding:32px 16px;font-family:Arial,sans-serif;color:#0f172a;">
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:28px;padding:32px;box-shadow:0 20px 60px rgba(15,23,42,0.08);">
        <p style="margin:0 0 12px;font-size:11px;letter-spacing:0.24em;text-transform:uppercase;color:#94a3b8;">Recurso gratuito</p>
        <h1 style="margin:0;font-size:34px;line-height:1.15;">${firstName}, ya tienes acceso a tu guia.</h1>
        <p style="margin:18px 0 0;font-size:16px;line-height:1.8;color:#475569;">
          Esta guia te ayuda a ordenar tu proceso comercial para atraer mejores oportunidades y cerrar mas ventas
          con menos improvisacion.
        </p>
        <div style="margin-top:28px;padding:24px;border-radius:22px;background:#fff7ed;border:1px solid #fdba74;">
          ...
          <p style="margin:0 0 10px;font-size:15px;line-height:1.7;">1. Como identificar el cuello de botella principal de tus ventas.</p>
          <p style="margin:0 0 10px;font-size:15px;line-height:1.7;">2. Como definir el siguiente paso comercial para cada lead.</p>
          <p style="margin:0;font-size:15px;line-height:1.7;">3. Como convertir ese proceso en una operacion mas ordenada con Nexora.</p>
        </div>
        <div style="margin-top:28px;display:flex;flex-wrap:wrap;gap:12px;">
          <a href="${links.aiStudioUrl}" style="display:inline-block;border-radius:999px;padding:12px 18px;background:#ea580c;color:#ffffff;text-decoration:none;font-weight:600;">Ver demo de Nexora</a>
  ...
  const text = [
    `Hola ${firstName},`,
    '',
    'Tu recurso gratuito ya esta listo.',
    '',
    'Aqui tienes una guia para ordenar tus oportunidades y mejorar conversiones con un proceso comercial mas claro.',
    '',
    `Ver demo de Nexora: ${links.aiStudioUrl}`,
    `Ver planes: ${links.pricingUrl}`,
    `${appLabel}: ${appHref}`,
    '',
    'Si quieres ayuda para implementarlo en tu negocio, responde este correo.',
  ].join('\n');
